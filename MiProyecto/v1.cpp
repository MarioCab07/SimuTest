...
#include <SFML/System.hpp> // Para sf::sleep y sf::milliseconds
#include <fstream>
#include <vector>
#include <sstream>
#include <iostream>
    ...
    while (std::getline(file, line))
    {
        ...
        while (iss >> token)
        {
            ...
            if (token == "#")
                ...
                cell.type = GRAVITY_DOWN, cell.shape.setFillColor(sf::Color(135, 206, 200));
            else if (token == "!")
            ...
            if (event.type == sf::Event::MouseButtonPressed)
            {
                ...
                if (mx >= 0 && my >= 0 && mx < cols && my < rows)
                {
                    ...
                    if (allowed && destino != WALL && !(destino == WALL_TOGGLE && turnos % 2 != 0))
                    {
                        ...
                        {
                            convertirAleatoriosEnMuros(grid, 1, playerX, playerY);
...
void showPath(std::vector<sf::Vector2i> &caminoFinal, std::vector<std::vector<Cell>> &grid, int gridSize, sf::RenderWindow &window)
{
    ...
    for (size_t i = 0; i < caminoFinal.size(); i++)
    {
        ...
        window.display();
        // Reemplaza std::this_thread::sleep_for(...) con la llamada de SFML:
